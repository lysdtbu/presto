...
class AhanaStatsReporter : public facebook::velox::BaseStatsReporter {
 public:
  AhanaStatsReporter(
    ...
    workerPod_ = worker.empty() ? std::getenv("HOSTNAME") : worker;
  }
   ...
   * and the labels method="POST" and handler="/messages"
   * could be written like this:
   *  # HELP num_tasks_aborted
   *  # TYPE num_tasks_aborted gauge*
   *  num_tasks_aborted{cluster="<cluster_id>", worker=""} value timestamp
   *
   * Above info is from:
